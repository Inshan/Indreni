        ...
        try {
            ...
            if (response.data) {
                const DocumentData = response.data.filter(
                    (item) => item.status === "Publish" && item.page_type === "MessagefromPresident"
                );
            ...
            <div className='relative w-full h-40 md:h-96 overflow-hidden'>
                ...
                <div className='w-full text-center absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 pt-12 md:pt-0'>
                    ...
                    <Breadcrumb className='text-gray-400 flex justify-center'>
                        ...
                        <BreadcrumbItem>
                            <BreadcrumbLink className=' text-xs md:text-sm'> {data.title}s </BreadcrumbLink>
                        </BreadcrumbItem>
                ...
                <div className='flex flex-col gap-10 md:flex-row'>
                    <div className='flex md:w-2/3 flex-col bg-gray-100 md:p-6'>
                        ...
                        <div className='order-1 md:order-2'>
                            <p className='py-4' dangerouslySetInnerHTML={{ __html: data.short_desc }}></p>
                        </div>
                    </div>
                    <div className='md:w-1/3  flex-col gap-4 md:gap-10 md:flex hidden'>
                        <SideNotice />
                    </div>
                </div>
            </div>
