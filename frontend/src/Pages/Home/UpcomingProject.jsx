        ...
        try {
            ...
            if (response.data) {
                const DocumentData = response.data.filter(
                    (item) => item.status === "Publish" && item.page_type === "Upcoming Project"
                );
            ...
            <div className=' py-4 xl:py-16'>
                <div className='container'>
                    ...
                    <div className='flex flex-col gap-4 md:gap-10 py-2 md:py-10 xl:flex-row'>
                        {data1.map((item) => (
                            <Link className='w-full bg-gray-200 group' to={`/upcomingprogram/${item.id}`}>
                    ...
                    <div className='w-full flex justify-center items-start pt-4'>
                        <Link to='/upcomingprogram' className='bg-blue py-2 px-6 text-white transition duration-300 hover:bg-secondaryblue hover:text-blue'>Load More</Link>
                    </div>
                </div>
            </div>
        </>
