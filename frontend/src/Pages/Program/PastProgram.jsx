...
const PastProgram = () => {
        ...
        try {
            ...
            if (response.data) {
                const DocumentData = response.data.filter(
                    (item) => item.status === "Publish" && item.page_type === "Past Program Submenu"
                );
    ...
    // const pastProject = projectData.filter((item) => item.projectCategory == 'past program')
    ...
    return (
        <>
            ...
            <div className='container py-4 md:py-16 flex flex-col gap-8'>
                {data1.map((item) => (
                    <Link className='group' to={`/pastprogram/${item.id}`}>
